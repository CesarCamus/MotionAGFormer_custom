...
def parse_args():
    ...
    parser.add_argument('--thred-score', type=float, default=0.90,
...
def model_load(config):
    ...
    state_dict = torch.load(config.OUTPUT_DIR,map_location='cpu')
...
def gen_video_kpts(video, det_dim=416, num_peroson=1, gen_output=False):
    ...
    for ii in range(video_length):
        ...
        if bboxs is None or not bboxs.any():
            ...
            if bboxs_pre is None or scores_pre is None:
                continue
            bboxs = bboxs_pre
            scores = scores_pre
        else:
